...
them -- the EDC/MDC stack plot (:class:`StackWindow`) and the two-dataset
comparison (:class:`CompareDialog`). MDC/EDC curve fitting has its own
panel, :mod:`ui.fit`.
...
class CompareDialog(QDialog):
    """Two datasets side by side, plus their difference or ratio.

    The comparison only means anything if both are on the same scale, so the
    shared levels are computed once for the pair and the difference is
    refused outright when the axes do not match -- a resampled difference of
    two different grids is a picture of the interpolation, not of the change.
    """
    ...
    def __init__(self, datasets, parent=None, colormap="gray", flip=False):
        super().__init__(parent)
        self.datasets = list(datasets)[:2]
        self.colormap, self.flip = colormap, flip
        self.setWindowTitle("Compare two datasets")
        self.resize(900, 620)

        layout = QVBoxLayout(self)
        (name_a, a), (name_b, b) = self.datasets
        self.a_values = np.asarray(a.scan.value, dtype=float)
        self.b_values = np.asarray(b.scan.value, dtype=float)
        self.axes = [np.asarray(a.scan.x, float), np.asarray(a.scan.y, float)]
        self.labels = [a.scan.labels.get("x", "x"), a.scan.labels.get("y", "y")]

        self.compatible = (self.a_values.shape == self.b_values.shape and
                           np.allclose(self.axes[0],
                                       np.asarray(b.scan.x, float), atol=1e-9) and
                           np.allclose(self.axes[1],
                                       np.asarray(b.scan.y, float), atol=1e-9))

        layout.addWidget(QLabel(f"<b>A</b> {name_a}<br><b>B</b> {name_b}"))
        self.preview = PreviewPair()
        self.preview.set_labels(*self.labels)
        self.preview.before["box"].setTitle("A")
        self.preview.after["box"].setTitle("B")
        layout.addWidget(self.preview, 1)
        ...
        form = QFormLayout()
        self.mode = QComboBox()
        self.mode.addItems(["A − B", "A / B", "(A − B) / (A + B)"])
        self.mode.currentIndexChanged.connect(self.refresh)
        form.addRow("Third panel", self.mode)
        self.shared = QCheckBox("Same levels for A and B")
        self.shared.setChecked(True)
        self.shared.toggled.connect(self.refresh)
        form.addRow("", self.shared)
        self.scale_first = QCheckBox("Scale B to A's mean first")
        self.scale_first.setToolTip(
            "Removes an overall intensity difference -- different exposure, "
            "different beam current -- so the panel shows a change of shape "
            "rather than a change of brightness.")
        self.scale_first.toggled.connect(self.refresh)
        form.addRow("", self.scale_first)
        ...
        buttons = QDialogButtonBox()
        add = buttons.addButton("Difference to list", QDialogButtonBox.ActionRole)
        add.clicked.connect(self.export_result)
        ...
        layout.addWidget(buttons)
        self.refresh()
        # This dialog fills both previews through _show directly (A and B
        # are two sources, not a source and a result), so the shape lock has
        # to be applied once here rather than by show_source.
        self.preview._apply_aspect()

    def _scaled_b(self):
        if not self.scale_first.isChecked():
            return self.b_values
        a_mean = np.nanmean(self.a_values)
        b_mean = np.nanmean(self.b_values)
        if not np.isfinite(b_mean) or abs(b_mean) < 1e-30:
            return self.b_values
        return self.b_values * (a_mean / b_mean)

    def result(self):
        if not self.compatible:
            raise ValueError(
                "These two are not on the same grid "
                f"({self.a_values.shape} against {self.b_values.shape}), so a "
                "difference would be a picture of the interpolation. Put them "
                "through the same conversion, or compress one to match.")
        b = self._scaled_b()
        mode = self.mode.currentIndex()
        with np.errstate(invalid="ignore", divide="ignore"):
            if mode == 0:
                return self.a_values - b
            if mode == 1:
                return np.where(np.abs(b) > 1e-30, self.a_values / b, np.nan)
            total = self.a_values + b
            return np.where(np.abs(total) > 1e-30,
                            (self.a_values - b) / total, np.nan)

    def refresh(self):
        a, b = self.a_values, self._scaled_b()
        if self.shared.isChecked():
            pool = np.concatenate([a[np.isfinite(a)].ravel(),
                                   b[np.isfinite(b)].ravel()])
            levels = (np.percentile(pool, [1, 99]) if pool.size else (0, 1))
        else:
            levels = None
        for target, values in ((self.preview.before, a), (self.preview.after, b)):
            self.preview._show(target, values, self.axes, self.colormap, self.flip)
        ...
        # A difference is signed, so it wants a symmetric window around zero
        # and (in the viewer) a diverging colormap -- an asymmetric one makes
        # a small positive change look like a large negative one.
        if self.mode.currentIndex() == 1:
            lo, hi = (np.percentile(finite, [2, 98]) if finite.size else (0, 2))
        else:
            reach = float(np.percentile(np.abs(finite), 98)) if finite.size else 1.0
            lo, hi = -reach, reach
        x, y = self.axes
        show_frame(self.result_view, result, x, y,
                   levels=(float(lo), float(hi)))
        apply_colormap(self.result_view, "redwhiteblue"
                       if self.mode.currentIndex() != 1 else self.colormap, False)
        fit_frame_view(self.result_view, x, y)
        self.note.setText(
            f"{self.mode.currentText()}: [{lo:.4g} .. {hi:.4g}], "
            f"RMS {np.sqrt(np.nanmean(finite ** 2)) if finite.size else float('nan'):.4g}")
    ...
    def export_result(self):
        try:
            ...
            return None
        (name_a, a), (name_b, _) = self.datasets
        label = {0: "diff", 1: "ratio", 2: "asym"}[self.mode.currentIndex()]
        params = {"a": name_a, "b": name_b, "mode": self.mode.currentText(),
                  "rescaled": self.scale_first.isChecked()}
        info = P.record_step(dict(a.scan.info), P.Step("compare", params,
                                                       source=f"{name_a} / {name_b}"))
        data = MemoryData("cut", tuple(self.axes), values, dict(a.scan.labels),
                          source_label=f"{name_a}_{label}",
                          parameters=params, prefix="proc.compare",
                          source_path=getattr(a, "path", ""), source_info=info)
        self.datasetsCreated.emit([data])
        self.note.setText(f"Added {data.source_label} to the list.")
        return data
    ...
    def to_figure(self):
        ...
        try:
            ...
            QMessageBox.warning(self, "Compare", str(exc))
            return None
        (name_a, _), (name_b, _) = self.datasets
        panels = []
        for values, name in ((self.a_values, name_a), (self._scaled_b(), name_b),
                             (result, self.mode.currentText())):
            panels.append(panel_from_arrays(values, self.axes[0], self.axes[1],
                                            self.labels[0], self.labels[1], name))
