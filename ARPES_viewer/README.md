# ARPES viewer -- a browser and analysis program for ARPES data

Python/PyQt5 program for ARPES data: browse the measurements in a folder,
open them in their own windows, convert to momentum, cut, fit, process and
lay the results out as a publication figure. It began as a rewrite of the
lab's `h5Loader*.py` for SOLEIL ANTARES `*.nxs` (NeXus/HDF5) files -- based
on the field layout documented in `load_soleil_nxs.m` -- and reading those
is now one loader among several rather than the whole program. It reads
SOLEIL ANTARES and SOLEIL CASSIOPEE, and its own saved format; adding a
beamline is a new file in `loader/`.
...
| `loader/soleil.py` | The SOLEIL/ANTARES reader. **Copy this file to add a beamline.** |
| `loader/cassiopee.py` | The SOLEIL/CASSIOPEE reader: Scienta text spectra, and folders of them assembled into a map or a photon-energy scan. |
...
| `tools/lattice.py`, `tools/spacegroups.py`, `tools/bz3d.py`, `tools/bz2d.py`, `tools/moire.py` | The Brillouin-zone geometry: lattices and point groups, the 3-D and 2-D zones and their irreducible wedges, and the moire zone. |
...
| `test/` | Run all of them with `python -m pytest` from this folder. |

Run with:
```
python ARPES_viewer.py
```
Requires: `PyQt5`, `pyqtgraph`, `numpy`, `h5py`, `Pillow`, `scipy` (matplotlib
optional, used only for a nicer thumbnail colormap). Nothing added since the
first version needs a new one.

Anything you compute in a session is written straight away to

```
~/.arpes_viewer/sessions/<date>-<pid>/
```

so that stopping the program does not lose it, and every write, save and
deletion is appended to `~/.arpes_viewer/sessions/operations.log` -- one
plain-text file, in the order things happened, which is what to open after a
crash. Those folders are working copies: a dataset's copy is deleted the
moment you save it somewhere of your own, whatever is left is pruned after
three days, and the program offers to reopen anything a previous run left
behind. Closing the main panel asks about anything not yet saved.

Session folders from before the rename (`~/.nxsloader/sessions`) are still
looked in, so upgrading does not hide a crashed run's work.

## Data kinds (mirrors the four cases in the .m file)

- `spem_4d`: real-space raster scan, full 4D cube (x, y, k, E). -> spatial
  map + **2D E-vs-k** view at the cursor.
- `spem_1d`: real-space *line* scan (x, k, E).
- `cut`: single deflector position -> one 2D E-vs-k spectrum.
- `map`: deflector-angle scan -> (deflx, k, E) cube -> converted to
  **(kx, ky, E)** and shown in the 3-panel slice explorer
  (`KCubeExplorer`), which also grows an optional true-3D OpenGL volume
  view if `pyqtgraph.opengl`/PyOpenGL happen to be installed.
- `unsupported`: case 1 (`Scan2D_MBS_vs_PIX_PIY`), same as the .m file
  ("not finish for 2019 version, dunt care anymore").
- Unrecognized top-level group name: raises with a clear message instead of
  silently mis-parsing; add the name to `GROUP_NAME_TO_CASE` once you know
  which case it matches (new beamline-software versions add new names --
  this already happened once, "AuK 2022" vs "2021").

## The one important thing to verify on real data

MATLAB's `h5read` reports HDF5 array dimensions **in reverse order**
compared to `h5py` (documented MATLAB behaviour, not a bug). That means
`load_soleil_nxs.m`'s `permute(value, [3 4 2 1])`-style calls cannot be
copied index-for-index into Python.

Instead of guessing a fixed transpose, `loader.nxs_file.align_and_transpose`
matches each raw array axis to a physical axis (x/y/k/E/deflx) **by length**
against the independently-read calibration arrays. This is robust unless
two axes happen to have the same number of points, in which case it warns
and picks one arbitrarily.

**Before trusting the output on your own files**, run:
```python
from nxs_file import inspect_nxs
inspect_nxs("your_file.nxs")
```
and compare the printed dataset shapes to the axis lengths reported for
that file's `NxsScan` (`scan.x.size`, `scan.y.size`, `scan.k.size`,
`scan.z.size`) -- if `align_and_transpose` ever warns about an ambiguous
match, that file is exactly where you'd want to look first. All four cases
were validated against synthetic files with deliberately-scrambled,
uniquely-sized axes (`test_process_nxs.py`), which exercises the matching
logic correctly, but a synthetic file obviously can't confirm the
*physical* axis order (which is x, which is y, ...) the real acquisition
software produces -- only a real file can.

## The other assumption worth knowing about: k conversion

For the `map` kind, the analyzer's own slit-direction axis (`scan.k`) is
already delivered in k (Å⁻¹) by the MBS acquisition software (this matches
the `.m` file's own `SPEM_kmin`/`SPEM_kmax` naming). The **deflector** axis
(`scan.x`, from the `actuator_1_1` scan) is a raw angle in degrees and is
converted to `kx` on demand by `to_kspace_cube()` using the standard
...
using `center_ke` from the file as the reference kinetic energy by default.
This ignores inner-potential/refraction correction and treats KE as constant
across the energy window -- fine for a narrow window around a feature of
interest, an approximation worth checking for a wide energy range (pass an
explicit `kinetic_energy_eV` array, one value per energy channel, to
`to_kspace_cube()` if you need per-channel accuracy).

## Thirtieth round: CASSIOPEE, and a folder that is one measurement
...
`load_Soleil_Cassiopee_folder_struct.m`.

### One spectrum

`[Region n]` with the two axis scales spelled out number by number,
`[Info n]` with the acquisition settings, `[Data n]` with one line per
energy point -- each line starting with its own energy, which repeats the
Dimension 1 scale and is dropped. It comes back as an ordinary `cut`:
analyser angle against kinetic energy, the same shape every other viewer,
fit and figure already works on.

Beside each spectrum sits an `_i` file with the motor positions. It is
parsed by splitting on the colon rather than at a fixed character offset as
the `.m` does -- the files are hand-formatted and the spacing varies, so a
field that moved by a character came back as a silent NaN. The colon has to
be the first one *outside brackets*, because the polarisation field's own
label lists its codes as `[0:LV, 1:LH, 2:AV, 3:AH, 4:CR]`. The first test
written against that line found the bug.

### A folder, and the thing the file does not say

A numbered folder is one measurement. Which one depends on what moved
between the spectra:

| what varies across the set | first axis | what it becomes |
|---|---|---|
| the sample's polar angle | theta (deg) | a Fermi-surface `map` |
| the monochromator | photon energy (eV) | an hv scan, energies referred to `E_F` |
| neither | cut index | repeated acquisitions, indexed |

The `.m` makes this decision with two `if` statements and then forgets it:
what comes back is a struct whose `x` is an angle or a photon energy, and
nothing downstream can tell which. Here the answer is recorded as
`axis0.role`, which is the field the k conversion reads -- so a
photon-energy scan is *refused* a conversion to in-plane momentum rather
than being given a perfectly plausible-looking wrong one. The Load window's
axis-role box gained an "As the file says" default so that this survives;
setting it explicitly still overrides, because a reader's guess is a guess.

For an hv scan the energy axis is referred to the Fermi level,
`E - E_F = E_kin - hv + phi`, using the beamline's measured work-function
table (45--170 eV). Outside that range the spline is extrapolated, as in
the original -- but it now warns, because a cubic through a table that dips
leaves its range quickly and a wrong work function shifts a whole scan's
binding energies while the picture still looks fine.
...
* **The median filter is off.** The `.m` ran a two-point median filter along
  ...
  averaged into a map forty times over.
* **A file name may contain underscores.** The `.m` splits names on `_` and
  ...
  `Map80eV_Phi18` -- there is a commented-out hand-set `name_base` in it
  that says so. Membership is matched here on the `_<n>_ROI<n>_` tail, so
  the base can be anything.
* **A scale that is genuinely not linear is reported.** Both loaders replace
  the printed axis by the straight line through it. On a real file the
  correction is about 1e-14 of a step, so it is defence against rounding --
  but if a scale ever *is* curved, straightening it silently moves the data
  onto the wrong energies.

### Reading a folder without freezing the window

A 40-cut folder is 240 MB of text and takes 3.5 s; a 100-cut one takes
about nine. That is far too long to spend on the GUI thread, and
`load_dataset` is called from eight places that all expect a value back
rather than a callback -- rewriting every one of them to be asynchronous
would be a much larger change than the problem justifies.

So `ui/jobs.py` gained `run_blocking`: the work runs on the worker thread as
usual, but the caller waits for the result while the event loop is pumped,
so the window keeps painting and the bar moves. What makes pumping the loop
safe here -- and not the `processEvents`-inside-a-computation pattern this
module was written to replace -- is that the progress dialog is
**application**-modal for the duration: the only thing that can be clicked
is Cancel, so there is no second handler to re-enter. Two things had to be
right for that: the runner reports itself busy during a blocking job (or a
timer firing in the pumped loop could start a second one and break the
one-job-at-a-time rule by way of the mechanism keeping the window alive),
...
Everything else keeps the direct path, which is both faster and a smaller
thing to get wrong.
...
* `load_soleil_nxs` refused any file not named `.nxs`, so a dataset this
  program saved as `.h5` could not be reopened. Latent, since the Save
  dialog forces `.nxs`, but the check was the wrong guard: detection is by a
  marker inside the file, and the extension test is there to catch a file of
  the wrong *type*. It now accepts the HDF5 family.

### New files

| | |
|---|---|
| `loader/cassiopee.py` | The reader: `parse_scienta`, the parameter file, the work-function table, `series_members`, `load_cut`, `load_series`. |
| `test/test_cassiopee.py` | 51 tests. The files are built in the test rather than shipped -- a real spectrum is 6 MB and what has to be checked is the format. |
